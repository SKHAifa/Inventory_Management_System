...
$productNameCol = in_array('name', $columns) ? 'name' : (in_array('product_name', $columns) ? 'product_name' : 'title');
$skuCol = in_array('sku', $columns) ? 'sku' : null;
$imageCol = in_array('image_name', $columns) ? 'image_name' : (in_array('image', $columns) ? 'image' : 'product_image');
...
$types = "";
if (!empty($search)) {
    $searchClauses = ["$productNameCol LIKE ?"];
    $params = ["%$search%"];
    $types = "s";
    if ($skuCol) {
        $searchClauses[] = "$skuCol LIKE ?";
        $params[] = "%$search%";
        $types .= "s";
    }
                        ...
                        <tbody>
                            <?php if ($result->num_rows > 0): ?>
                                <?php while ($product = $result->fetch_assoc()): ?>
                                    ...
                                    if (!empty($safeImage) && file_exists('../images/' . $safeImage)) {
                                        $imagePath = '../images/' . htmlspecialchars($safeImage);
                                    } elseif (!empty($safeImage) && file_exists('../uploads/' . $safeImage)) {
                                        $imagePath = '../uploads/' . htmlspecialchars($safeImage);
                                    } else {
                                        $imagePath = 'https://via.placeholder.com/70x70/6B7280/FFFFFF?text=No+Image';
                                    }
                                    ?>
                                    <tr class="<?= ($product['quantity'] ?? 0) < 10 ? 'table-danger' : (($product['quantity'] ?? 0) < 20 ? 'table-warning' : '') ?>">
                                        ...
                                        <td>
                                            <code class="bg-light px-2 py-1 rounded fs-6">
                                                <?= $skuCol ? htmlspecialchars($product[$skuCol] ?? 'N/A') : 'N/A' ?>
                                            </code>
                                        </td>
